...
def trouver_debut_bloc_code(message): return re.sub(r"\n", "", re.search(r"'''(\w+)", message).group(0)) if re.search(r"'''(\w+)", message) else False
def trouver_fin_bloc_code(message): return re.sub(r"\n", "", re.search(r"'''(?=\s|\n|$)", message).group(0)) if re.search(r"'''(?=\s|\n|$)", message) else False
